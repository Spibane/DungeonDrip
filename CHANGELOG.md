...
- **Added gear options to the game's own right-click menus.** Right-click a piece of gear in your inventory, the armoury chest, the inspect window, a chat link, the Glamour Dresser, a vendor or the market board and Dungeon Drip's options are there: **Try on outfit** for each set the piece belongs to, and **Where does this drop?**. Only what the game does not already offer is added - it has its own Try On, Link and Copy, and a second of each would sit beside the real one saying the same word. Entries go through the interface Dalamud provides for this, are marked with a D, and sit below the game's own; turn them off under Settings > In-game UI
- Added **Where does this drop?** to the right-click menu on any gear row, listing the duties that drop it and taking you to whichever you pick. Every answer says how much of the wiki has been read, so a short list is not mistaken for a complete one, and a piece nothing lists says exactly that rather than claiming it drops nowhere - loot coverage for new content is thin by design, since the wiki is only read for duties you have opened
