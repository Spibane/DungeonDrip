...
- Changed the vendor panel to open at the shop window's height with the list scrolling inside, instead of growing to fit its contents. A long stock list used to run off the bottom of the screen, and since the panel cannot be dragged out of the way there was no recovering from it. The width fits the longest item name. The panel is now resizable, remembers the size you drag it to, and offers a button to go back to matching the shop
